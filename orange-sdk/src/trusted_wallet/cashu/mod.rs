...
pub struct Cashu {
	...
	_store: Arc<dyn KVStore + Send + Sync>,
...
impl TrustedWalletInterface for Cashu {
	type ExtraConfig = CashuConfig;

	fn init(
		config: &WalletConfig<CashuConfig>, store: Arc<dyn KVStore + Sync + Send>,
		_event_queue: Arc<EventQueue>, logger: Arc<Logger>,
		...
		async move {
			...
			// TODO: Implement proper persistent storage using CashuKvDatabase
			...
			let cashu_wallet = Arc::new(
				Wallet::new(
					&config.extra_config.mint_url,
					config.extra_config.unit.clone(),
					db,
			...
			let (mint_quote_sender, mut mint_quote_receiver) =
				mpsc::channel::<MintQuote>(32);

			// Start mint quote monitoring task
			let wallet_for_monitoring = Arc::clone(&cashu_wallet);
			let event_queue_for_monitoring = Arc::clone(&_event_queue);
			...
			if let Ok(pending_mints) = cashu_wallet.get_active_mint_quotes().await {
				for pending_mint in pending_mints {
					if let Err(e) = mint_quote_sender.send(pending_mint.clone()).await {
						log_error!(logger, "Failed to send pending mint quote {} for monitoring: {e}", pending_mint.id);
					}
				}
			}
	...
	fn get_reusable_receive_uri(
		...
		async move {
			...
			if let Err(e) = self.mint_quote_sender.send(mint_quote.clone()).await {
				log_error!(self.logger, "Failed to send mint quote {} for monitoring: {e}", mint_quote.id);
			}
			...
			match amount {
				...
				Some(a) => {
					...
					if let Err(e) = self.mint_quote_sender.send(quote.clone()).await {
						log_error!(self.logger, "Failed to send mint quote {} for monitoring: {e}", quote.id);
					}
	...
	fn list_payments(&self) -> impl Future<Output = Result<Vec<Payment>, TrustedError>> + Send {
		async move {
			...
			let payments = transactions
				.into_iter()
				.map(|tx| Self::convert_transaction_to_payment(tx))
...
impl Cashu {
	...
	fn convert_transaction_to_payment(transaction: Transaction) -> Result<Payment, TrustedError> {
		...
		// Convert amounts - CDK amounts are u64 representing satoshis
		let amount =
		...
		let outbound =
			if transaction.direction == TransactionDirection::Outgoing { true } else { false };
