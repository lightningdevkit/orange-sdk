	...
	pub(crate) async fn handle_ldk_node_event(&self, event: ldk_node::Event) {
		match event {
			ldk_node::Event::PaymentSuccessful {
				...
				if self
					.tx_metadata
					.set_preimage(payment_id, payment_hash.0, preimage.0)
					.await
					.is_err()
				{
			...
			ldk_node::Event::SpliceNegotiationFailed { user_channel_id, .. } => {
				...
				if !self.splice_pending_inbox.fail(user_channel_id.0) {
					log_debug!(
						self.logger,
						"No local splice operation is waiting on channel {}; ignoring failure",
						user_channel_id.0
					);
				}
			},
