...
pub async fn build_test_nodes() -> TestParams {
	...
	wait_for_condition(Duration::from_secs(1), 10, "third_party node sync after funding", || {
	...
	#[cfg(feature = "_cashu-tests")]
	{
		...
		let mint = {
			...
			// need to convert to cdk's SocketAddress
			use std::net::ToSocketAddrs;
			let listen = lsp_listen_clone.to_socket_addrs().unwrap().next().unwrap();

			cdk.node()
				.open_channel(lsp_node_id, listen.into(), 10_000_000, Some(5_000_000_000), None)
				.unwrap();
			// wait for tx to broadcast
