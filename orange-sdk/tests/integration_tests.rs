...
async fn test_receive_to_trusted() {
	...
	wallet.stop().await;
	debug!("stopping third party");
	third_party.stop().unwrap();
	debug!("stopping lsp");
	lsp.stop().unwrap();
}
...
async fn test_threshold_boundary_trusted_balance_limit() {
	...
	match event {
		Event::PaymentReceived { amount_msat, lsp_fee_msats, .. } => {
			assert_eq!(amount_msat + lsp_fee_msats.unwrap_or(0), above_limit_amount.milli_sats());
		},
		e => panic!("Expected PaymentReceived event, got {e:?}"),
	}
...
async fn test_payment_network_mismatch() {
	...
	let instr =
		PaymentInstructions::parse(wrong_network, Network::Bitcoin, &HTTPHrnResolver::new(), true)
			.await
			.unwrap();
