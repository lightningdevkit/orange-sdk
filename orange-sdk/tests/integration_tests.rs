...
async fn test_sweep_to_ln() {
	...
	let limit = wallet.get_tunables();
	let recv_amt = Amount::from_milli_sats(limit.trusted_balance_limit.milli_sats() / 2).unwrap();
...
async fn test_force_close_handling() {
	...
	lsp.force_close_channel(&channel.user_channel_id, channel.counterparty_node_id, None).unwrap();
...
async fn test_threshold_boundary_trusted_balance_limit() {
	...
	test_utils::wait_for_condition(Duration::from_secs(1), 10, "exact limit payment", || async {
...
async fn test_threshold_boundary_rebalance_min() {
	...
	test_utils::wait_for_condition(Duration::from_secs(1), 10, "wait for transaction", || async {
...
async fn test_threshold_boundary_onchain_receive_threshold() {
	...
	assert!(uri.invoice.amount_milli_satoshis().is_some(), "Should also include Lightning invoice");
...
async fn test_threshold_combinations_and_edge_cases() {
	...
	assert!(uri.invoice.amount_milli_satoshis().is_some(), "Should handle minimum 1 sat amount");
...
async fn test_invalid_payment_instructions() {
	...
	let desc = Bolt11InvoiceDescription::Direct(Description::empty());
	let invoice = third_party.bolt11_payment().receive(amount.milli_sats(), &desc, 300).unwrap();
...
async fn test_payment_with_expired_invoice() {
	...
	test_utils::wait_for_condition(Duration::from_secs(1), 10, "wallet balance update", || async {
...
async fn test_payment_network_mismatch() {
	...
	let instr = PaymentInstructions::parse(wrong_network, Network::Bitcoin, &HTTPHrnResolver, true)
		.await
		.unwrap();
...
async fn test_concurrent_payments() {
	...
	assert!(
		...
		total_payment_amount.saturating_add(Amount::from_sats(1000).unwrap()).sats().unwrap_or(0)
	);
	...
	assert!(result1.is_ok(), "First concurrent payment initiation should succeed: {:?}", result1);
	assert!(result2.is_ok(), "Second concurrent payment initiation should succeed: {:?}", result2);
	assert!(result3.is_ok(), "Third concurrent payment initiation should succeed: {:?}", result3);
	...
	assert!(balance_decrease > Amount::ZERO, "Balance should decrease after successful payments");
...
async fn test_balance_consistency_under_load() {
	...
	for b in &balances[1..] {
		...
		assert_eq!(
			...
			first_balance.available_balance(),
...
async fn test_invalid_tunables_relationships() {
	...
	assert!(uri_below.invoice.amount_milli_satoshis().is_some(), "Below limit should have invoice");
	assert!(uri_above.invoice.amount_milli_satoshis().is_some(), "Above limit should have invoice");
	...
	if below_limit >= current_tunables.onchain_receive_threshold {
		assert!(uri_below.address.is_some(), "Amount >= onchain_threshold should include address");
	}
	if above_limit >= current_tunables.onchain_receive_threshold {
		assert!(uri_above.address.is_some(), "Amount >= onchain_threshold should include address");
	}
}
...
async fn test_extreme_amount_handling() {
	...
	for amount in large_amounts {
		...
		let uri = uri_result.unwrap();
		assert!(uri.invoice.amount_milli_satoshis().is_some(), "Large amount should have invoice");
...
async fn test_edge_case_payment_instruction_parsing() {
	...
	let special_chars = ["lightning:", "bitcoin:?lightning=", "LIGHTNING:", "BITCOIN:?LIGHTNING="];
...
async fn test_lsp_connectivity_fallback() {
	...
	let inv = third_party
		.bolt11_payment()
		.receive(amount.milli_sats(), &Bolt11InvoiceDescription::Direct(Description::empty()), 300)
		.unwrap();
