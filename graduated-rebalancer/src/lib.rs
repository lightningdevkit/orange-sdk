...
/// Information about a pending payment found in the trusted wallet
#[derive(Debug, Clone, Copy)]
pub struct PendingPaymentInfo {
	/// The payment ID assigned by the trusted wallet
...
pub trait TrustedWallet: Send + Sync {
	...
	/// Returns `Some(PendingPaymentInfo)` if a pending payment with the given hash exists,
...
pub trait LightningWallet: Send + Sync {
	...
	/// Find an untracked pending channel/splice with the LSP.
...
> {
	...
	active_trusted_rebalance: Arc<tokio::sync::Mutex<Option<RebalanceState>>>,
	...
	L: Logger + Send + Sync + 'static,
{
	...
	async fn do_trusted_rebalance(&self, params: TriggerParams) {
		...
		let mut rebalance = self.active_trusted_rebalance.lock().await;
	...
	async fn do_onchain_rebalance(&self, params: TriggerParams) {
		let mut onchain_rebalance = self.active_onchain_rebalance.lock().await;
	...
	async fn complete_onchain_rebalance(
		&self, state: OnChainRebalanceState,
		onchain_rebalance: &mut tokio::sync::MutexGuard<'_, Option<OnChainRebalanceState>>,
	) {
		log_info!(
			self.logger,
	...
	async fn complete_rebalance(
		&self, state: RebalanceState,
		rebalance: &mut tokio::sync::MutexGuard<'_, Option<RebalanceState>>,
	) {
		log_info!(
			self.logger,
	...
	///
	/// Returns true if you should call [`Self::do_trusted_rebalance_if_needed`] again
	/// to retry any cleaned up rebalances.
	pub async fn recover_incomplete_trusted_rebalances(&self) -> Result<bool, ()> {
		...
		if let Some(mut state) = state_opt {
			...
			log_debug!(
				self.logger,
				"Recovering rebalance {} (ln_received: {}, trusted_sent: {}, trusted_payment_id: {:?})",
			...
			// persisting the trusted_payment_id. However, the payment might have actually
			// been sent, we need to check with the trusted wallet.
			if state.trusted_payment_id.is_none() {
				log_info!(
					self.logger,
					"Rebalance {} has no trusted_payment_id, checking if payment was actually sent",
					state.expected_payment_hash.as_hex()
				);

				match self.trusted.find_payment_by_hash(state.expected_payment_hash).await {
					Ok(Some(pending_info)) => {
						// Payment was actually sent, Recover the payment id and continue.
						log_info!(
							self.logger,
							"Found pending payment {} for rebalance {}, recovering",
							pending_info.payment_id.as_hex(),
							state.expected_payment_hash.as_hex()
						);
						state.trusted_payment_id = Some(pending_info.payment_id);
						// Update persistence with the recovered payment id.
						self.persistence.update_trusted_rebalance_state(state).await;
					},
					Ok(None) => {
						// The payment was never actually sent. Clean up and retry.
						log_info!(
							self.logger,
							"Rebalance {} was never initiated (no payment found), cleaning up and retrying",
							state.expected_payment_hash.as_hex()
						);
						self.persistence.remove_trusted_rebalance_state().await;
						return Ok(true);
	...
	/// This should be called during wallet initialization
	///
	/// Returns true if you should call [`Self::do_onchain_rebalance_if_needed`] again
	/// to retry any cleaned up rebalances.
	pub async fn recover_incomplete_onchain_rebalances(&self) -> Result<bool, ()> {
		...
		if let Some(mut state) = state_opt {
			...
			if let Some(user_channel_id) = state.user_channel_id {
				...
				// user_channel_id not set yet, we crashed after persisting state but before
				// persisting the user_channel_id. However, the channel/splice might have actually
				// been initiated, we need to check with the lightning wallet.
				log_info!(
					self.logger,
					"On-chain rebalance has no user_channel_id, checking if channel was actually opened"
				);

				match self.ln_wallet.find_pending_lsp_channel() {
					Some(user_channel_id) => {
						...
						// Check if it's already pending/ready
						if let Some(outpoint) = self.ln_wallet.get_channel_outpoint(user_channel_id)
						{
							log_info!(
								self.logger,
								"Recovered channel {user_channel_id} is already pending, finalizing"
							);
							state.pending_confirmed = true;
							state.channel_outpoint = Some(outpoint);
							*onchain_rebalance = Some(state);
							...
							// Still waiting for channel pending
							*onchain_rebalance = Some(state);
						}
					},
					None => {
						// Pending channel not found, the splice/channel open was never initiated.
						// Clean up and retry.
						log_info!(
							self.logger,
							"On-chain rebalance was never initiated (no pending channel found), cleaning up and retrying"
						);
						self.persistence.remove_onchain_rebalance_state().await;
						return Ok(true);
					},
	...
	pub async fn stop(&self) {
		log_debug!(self.logger, "Waiting for balance mutexes...");
		let _ = tokio::join!(
			self.active_trusted_rebalance.lock(),
			self.active_onchain_rebalance.lock()
		);
	}
}
